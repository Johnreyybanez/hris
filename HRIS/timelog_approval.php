  ...
  if (selectedDepartment === '') {
    ...
    $.fn.dataTable.ext.search.push(
      function departmentFilter(settings, data, dataIndex) {
        if (settings.nTable.id !== 'timelogTable') {
          return true;
        }
...
$('#statusFilter').on('change', function() {
  ...
  $.fn.dataTable.ext.search = $.fn.dataTable.ext.search.filter(function(fn) {
    return fn.toString().indexOf('timelogStatusFilter') === -1;
  });
  ...
  if (selectedStatus === '') {
    ...
    // Add custom search function using status column
    $.fn.dataTable.ext.search.push(
      function timelogStatusFilter(settings, data, dataIndex) {
        if (settings.nTable.id !== 'timelogTable') {
          return true;
        }
        // Get status from the status column (index 6)
        var status = data[6].includes(selectedStatus);
        return status;
      }
    );
    table.draw();
        ...
        $.ajax({
          ...
          success: function(response) {
            if (response.trim() === 'success') {
              Swal.fire('Deleted!', 'Timelog request has been deleted.', 'success');
              row.remove();
            } else {
