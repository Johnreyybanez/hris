...
$user_image = 'assets/images/logo-dark.svg'; // fallback image
$user_name = 'Guest';
$notifications = [];
$unread_count = 0;
...
if (isset($_SESSION['login_id'])) {
    ...
    $notifications_sql = "SELECT id, message, is_read, created_at FROM notifications WHERE user_id = ? ORDER BY created_at DESC LIMIT 5";
    if ($notif_query = $conn->prepare($notifications_sql)) {
        $notif_query->bind_param("i", $login_id);
        $notif_query->execute();
                      ...
                      <div class="notification-item <?= !$notif['is_read'] ? 'unread' : '' ?>">
                          <div class="d-flex align-items-start">
                              ...
                              <div class="flex-grow-1 ms-3">
                                  ...
                                  <span class="text-muted smaller"><?= date('M d, h:i A', strtotime($notif['created_at'])) ?></span>
                              </div>
                          </div>
                      </div>
                  <?php endforeach; ?>
