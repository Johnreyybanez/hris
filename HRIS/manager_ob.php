    ...
    if (!empty($time_from) && !empty($time_to) && strtotime($time_from) >= strtotime($time_to)) {
        ...
        if ($emp_result->num_rows == 0) {
            ...
            if ($stmt->execute()) {
                ...
                $stmt->close();
                $error_message = "Error adding OB request: " . $conn->error;
            }
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_ob'])) {
    ...
    if ($stmt->execute()) {
        ...
        $stmt->close();
        $error_message = "Error updating OB request: " . $conn->error;
    }
}
...
// Modified SQL query to show only manager's OB requests
$sql = "SELECT 
    ob.ob_id,
...
<div class="pc-container">
  <div class="pc-content">
    ...
    <table id="obTable" class="table table-bordered table-striped">
        ...
        <tbody>
            <?php while ($row = $result->fetch_assoc()): ?>
                <tr>
                    ...
                    <td>
                        <button class="btn btn-sm btn-primary btn-edit-ob"
                            data-ob='<?= htmlspecialchars(json_encode($row), ENT_QUOTES, 'UTF-8') ?>'>
                            <i class="bi bi-pencil-square"></i>
                        </button>
                        <button class="btn btn-sm btn-danger btn-delete-ob" data-ob-id="<?= $row['ob_id'] ?>">
