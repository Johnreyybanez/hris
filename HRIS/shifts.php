...
if (isset($_POST['add_shift'])) {
  ...
  $insert = mysqli_query($conn, "INSERT INTO shifts 
      (shift_name, time_in, break_out, break_in, time_out, is_flexible, has_break, description) 
      VALUES 
      ('$name', '$time_in', '$break_out', '$break_in', '$time_out', $is_flexible, $has_break, '$description')");

  $_SESSION['success'] = $insert ? 'Shift added successfully!' : 'Failed to add shift.';
...
if (isset($_POST['update_shift'])) {
  ...
  $update = mysqli_query($conn, "UPDATE shifts SET 
      shift_name='$name', time_in='$time_in', break_out='$break_out', break_in='$break_in', 
      time_out='$time_out', is_flexible=$is_flexible, has_break=$has_break, description='$description'
      WHERE shift_id=$id");

  $_SESSION['success'] = $update ? 'Shift updated successfully!' : 'Failed to update shift.';
  header("Location: shifts.php");
  exit();
}
...
if (isset($_POST['delete_shift'])) {
  ...
  $_SESSION['success'] = $delete ? 'Shift deleted successfully!' : 'Failed to delete shift.';
  header("Location: shifts.php");
  exit();
}
...
function calculate_total_hours($time_in, $break_out, $break_in, $time_out) {
  ...
  if (!empty($break_out) && !empty($break_in)) {
    ...
    // Handle overnight breaks (rare but possible)
        ...
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            <div>
              ...
              <small class="text-muted">Manage shift schedules and times</small>
            </div>
            ...
            <div class="table-responsive">
              <table id="shiftTable" class="table table-hover">
                ...
                <tbody>
                  ...
                  ?>
                  <tr>
                    <td><span class="badge bg-light-primary"><?= $row['shift_id']; ?></span></td>
                    <td><?= htmlspecialchars($row['shift_name']); ?></td>
                    <td><?= date('h:i A', strtotime($row['time_in'])) . ' - ' . date('h:i A', strtotime($row['time_out'])); ?></td>
                    <td><?= number_format($total_hours, 1); ?> hours</td> <!-- 👈 NEW -->
  ...
  <div class="modal-dialog">
    <form method="POST" class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="mb-3">
          <label for="add_break_out" class="form-label">Break Out</label>
          <input type="time" name="break_out" id="add_break_out" class="form-control">
        </div>

        <div class="mb-3">
          <label for="add_break_in" class="form-label">Break In</label>
  ...
  <div class="modal-dialog">
    <form method="POST" class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="mb-3">
          <label for="edit_break_out" class="form-label">Break Out</label>
          <input type="time" name="break_out" id="edit_break_out" class="form-control">
        </div>

        <div class="mb-3">
          <label for="edit_break_in" class="form-label">Break In</label>
