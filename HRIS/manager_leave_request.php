...
// Get leave requests for the manager
    ...
    <tbody>
      <?php while ($row = mysqli_fetch_assoc($leave_q)): ?>
        <tr>
          ...
          <td data-toggle="tooltip" title="<?= date('D, M d, Y', strtotime($row['start_date'])) ?>">
            <?= date('Y-m-d', strtotime($row['start_date'])) ?>
          </td>
          <td data-toggle="tooltip" title="<?= date('D, M d, Y', strtotime($row['end_date'])) ?>">
...
$(document).ready(function () {
    ...
    const table = $('#leaveTable').DataTable({
        ...
        pageLength: 10,
        order: [[5, 'desc']], // Order by requested date
        language: {
            search: "_INPUT_",
        ...
        columnDefs: [
            {
                targets: 3, // Total Days column
                type: 'num',
                render: function(data, type, row) {
                    if (type === 'display') {
                        // Extract the actual numeric value from the data-sort attribute or parse from text
                        const $cell = $(data);
                        let totalDays, actualDays;
                        
                        if ($cell.length && $cell.attr('data-total')) {
                            // If it's already a jQuery object with data attributes
                            totalDays = parseFloat($cell.attr('data-total'));
                            actualDays = parseFloat($cell.attr('data-actual'));
                        } else {
                            // Parse from the current cell content
                            const $currentCell = $(row).find('td').eq(3);
                            const $span = $currentCell.find('span');
                            totalDays = parseFloat($span.attr('data-total'));
                            actualDays = parseFloat($span.attr('data-actual'));
                        }
                        
                        const formattedTotal = formatLeaveDays(totalDays);
                        const formattedActual = formatLeaveDays(actualDays);
                        
                        return `<span data-total="${totalDays}" data-actual="${actualDays}" data-toggle="tooltip" title="Actual: ${formattedActual}">${formattedTotal}</span>`;
                    } else if (type === 'sort') {
                        // For sorting, use the numeric value
                        const $cell = $(data);
                        if ($cell.length && $cell.attr('data-total')) {
                            return parseFloat($cell.attr('data-total'));
                        }
                        // Extract numeric value from the cell's data-sort attribute
                        const sortValue = $(row).find('td').eq(3).attr('data-sort');
