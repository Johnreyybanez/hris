...
if (isset($_POST['add_loan'])) {
    ...
    $insert = mysqli_query($conn, "INSERT INTO LoanTypes (name, description) VALUES ('$name', '$description')");
    $_SESSION['success'] = $insert ? 'Loan type added successfully!' : 'Failed to add loan type.';
...
if (isset($_POST['update_loan'])) {
    ...
    $update = mysqli_query($conn, "UPDATE LoanTypes SET name='$name', description='$description' WHERE loan_type_id=$id");
    $_SESSION['success'] = $update ? 'Loan type updated successfully!' : 'Failed to update loan type.';
    header("Location: loan_types.php");
    exit();
}
...
if (isset($_POST['delete_loan'])) {
    $id = $_POST['delete_id'];
    $delete = mysqli_query($conn, "DELETE FROM LoanTypes WHERE loan_type_id=$id");
    $_SESSION['success'] = $delete ? 'Loan type deleted successfully!' : 'Failed to delete loan type.';
    header("Location: loan_types.php");
    exit();
}
?>
        ...
        <div class="row align-items-center">
          <div class="col-md-12">
            <div class="page-header-title">
              <h5 class="m-b-10">Loan Type Management</h5>
            </div>
            <ul class="breadcrumb">
        ...
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            <div>
              ...
              <small class="text-muted">Manage employee loan categories</small>
            </div>
            <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addModal">
              <i class="ti ti-plus me-1"></i>Add Loan Type
            </button>
          </div>
          <div class="card-body">
            <div class="table-responsive">
              <table id="typeTable" class="table table-hover">
                ...
                <tbody>
                  <?php
                  $result = mysqli_query($conn, "SELECT * FROM LoanTypes ORDER BY loan_type_id ASC");
                  while ($row = mysqli_fetch_assoc($result)): ?>
                  <tr>
                    <td><span class="badge bg-dark -secondary"><?= $row['loan_type_id']; ?></span></td>
  ...
  <div class="modal-dialog">
    <form method="POST" class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title"><i class="ti ti-plus me-2"></i>Add Loan Type</h5>
      ...
      <div class="modal-footer">
        <button type="submit" name="add_loan" class="btn btn-primary"><i class="ti ti-check me-1"></i>Add</button>
      </div>
    </form>
  </div>
</div>
...
<div class="modal fade" id="editModal" tabindex="-1">
  <div class="modal-dialog">
    <form method="POST" class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title"><i class="ti ti-edit me-2"></i>Edit Loan Type</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body">
      ...
      <div class="modal-footer">
        <button type="submit" name="update_loan" class="btn btn-warning"><i class="ti ti-device-floppy me-1"></i>Update</button>
      </div>
    </form>
  </div>
</div>
...
$(document).ready(function () {
  const table = $('#typeTable').DataTable({
    ...
    language: {
      ...
      emptyTable: "No loan types available",
